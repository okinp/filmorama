...
const imagePathReducer = path => (acc, curr) => {
  let imageUrl = path ? imagePath + curr + path : "https://via.placeholder.com/1280x720";
...
const movieBasicInfoFn = function(movieData){
  movieData.posters = posterSizes.reduce( imagePathReducer(movieData.poster_path), {} );
  movieData.backdrops = backdropSizes.reduce( imagePathReducer(movieData.backdrop_path), {} );
  ...
  let { vote_count, video, popularity, poster_path, original_language, 
        original_title, genre_ids, backdrop_path, adult, release_date, ...rest } = movieData;
